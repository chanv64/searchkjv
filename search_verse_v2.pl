...
if ($info) {
#	print_info(%hash);
	print_info_length(%hash);

#	cmpthese -1 => {
#		info 		=>  sub {print_info(%hash)},
#		infolen		=>  sub {print_info_length(%hash)},
#	};
	exit;
}
...
if ($printv) {
	...
	if ((defined $bbook) && (defined $verse)) {
		$selectedbook = book_exist($bbook,%hash); 
		if ($selectedbook ne "") {
#for testing
#			my $bindex=1;
#			foreach my $key (keys %hash ) {
#				print "$bindex. ";
#				print_verse($hash{$key}{Abbr},$verse,%hash);
#				$bindex++;
#			}
			print_verse($bbook,$verse,%hash);
		}
		else {
...
if ($lastverse) {
	$selectedbook = book_exist($strSearch,%hash); 
	if ($selectedbook ne "") {
#		print "Selected Book = $selectedbook\n";
		print "Last Verse of Book $selectedbook\n";
		print print_last_verse($selectedbook,%hash);
	}
	else {
...
if ($abook) {
	...
	$selectedbook = book_exist($bb,%hash); 
	if ($selectedbook ne "") {
		print_book($selectedbook,$ch,%hash);
	}
	else {
		print $strSearch," not found\n";
		print "These are the valid books\n";
